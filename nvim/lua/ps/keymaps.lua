...
local function BufNext()
    vim.cmd('bnext')
end

local function BufPrev()
    vim.cmd('bnext')
end
...
nmap('gn', BufNext, { desc = "Buffer Next" })
...
nmap('<leader>tl', alib.action_func('tmux-sessions'), {
    desc = 'Tmux Sessions'
})
...
-- nmap ('<C-n>', ':FloatermToggle main<cr>')
...
nmap('<leader>tp', function() vim.cmd 'tabprev' end, {
    desc = 'Tab prev'
})
